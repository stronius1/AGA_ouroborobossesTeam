...
![>Управление версиями](@document/archtool.archver)
...
SEAF.ArchTool предлагает отказаться от использования визуальных редакторов в пользу описания архитектуры кодом. Использовать
...
* [Манифесты](/docs/archtool.contexts) - структурированные файлы в формате YAML/JSON для описания архитектурных объектов. 
...
![>Управление децентрализованной архитектурой](@document/archtool.archdistrib)
...
![>Структура холдинга](@document/archtool.archeco)
...
Таким образом, управление архитектурой экосистемы является нетривиальной задачей. SEAF.ArchTool предлагает и тут использовать
принципы развития кодовой базы Open Source сообществами.

![<Управление архитектурой холдинга](@document/archtool.archeco.proc)
...
SEAF.ArchTool используется как универсальное средство представления артефактов экосистемы.

![](@anchor/facade)
## 4. Архитектурные фасады

![>Архитектурные фасады](@document/archtool.facade)
...
SEAF.ArchTool с этой задачей хорошо справляется. Он может быть развернут в качестве архитектурного фасада. Внешние 
пользователи получают удобный интерфейс для изучения публичной архитектуры.

![](@anchor/mindmap)

## 5. Mind Map
Функциональность SEAF.ArchTool можно представить Mind Map:
![Карта памяти SEAF.ArchTool](@document/archtool.mindmap)

## 6. Поставка
SEAF.ArchTool поставляется в составе двух компонентов в открытых исходных кодах:
1. WEB портал - [https://gitverse.ru/seafteam/seaf-archtool-core](https://gitverse.ru/seafteam/seaf-archtool-core)
2. Плагин для IDE JetBrains - [https://gitverse.ru/seafteam/seaf-archtool-ideplugin-jetbrains](https://gitverse.ru/seafteam/seaf-archtool-ideplugin-jetbrains)
