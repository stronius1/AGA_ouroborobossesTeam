...
Ядро SEAF.ArchTool предоставляет два интерфейса для взаимодействия плагинов с ним:
...
* **component** - Vue 3 компонент, который будет использован для рендеринга документов указанного типа.
...
docs:
  ...
  arcthool.plugins.example:
    location: SEAF.ArchTool/Руководство/Документы/HTML
    type: html
...
Пример встраивания документа в [markdown](/docs/archtool.markdown):

```
![Пример документа плагина](@document/archtool.plugins.example?param1=foo1&param2=foo2)
```
...
[/docs/archtool.plugins.example?param1=foo1&param2=foo2](/docs/archtool.plugins.example?param1=foo1&param2=foo2)
...
docs:
    ...
    ...
```
Вызовет компонент плагина с параметром path содержащим строку "/docs/archtool.plugins.example".
...
* **expression** - [JSONata](https://jsonata.org/) запрос к архитектуре или идентификатор [DataSet](/docs/archtool.datasets). 
