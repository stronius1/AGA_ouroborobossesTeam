...
Метамодель SEAF.ArchTool можно расширять. Это позволяют делать кастомные сущности.
...
Данная запись сообщила SEAF.ArchTool, что в метамодели появилась новая сущность "interactions". 
...
SEAF.ArchTool, на этом этапе, ничего о ней не знает и делать с ней не умеет.


## Схема объектов сущности

Для того, чтобы SEAF.ArchTool мог контролировать структуру и наполнение объектов сущности "interactions", необходимо 
...
entities:
  interactions:
    ...
    schema:
      ...
      # состоящего из доменов разделенных точкой. Например: archtool.user
...
Теперь SEAF.ArchTool знает как должна быть описана структура в секции "interactions" и может подсказывать при вводе кода 
в [плагине IDE](https://gitverse.ru/seafteam/seaf-archtool-core/releases):
...
Для генерации используются [документы](/docs/archtool.docs). Вы можете использовать любой 
тип документа доступный в SEAF.ArchTool.

Для начала создадим карточку объекта. В ней будет отражаться диаграмма взаимодействия с использованием
[PlantUML](/docs/archtool.plantuml).
...
Подробнее о шаблонах можно узнать [здесь](/docs/archtool.templates). 
...
если в параметре "id" будет передано значение "archtool.user", то запрос вернут следующую структуру:
...
```
![](@entity/interactions/blank?id=archtool.user)
```
        ...
        (
          {
            ...
            "link": "/entities/interactions/blank?id=archtool.user"
          }
        )
      ...
```

сообщит SEAF.ArchTool, что необходимо в меню добавить пункт "Взаимодействия"->"Пример", который ведет на объект 
с идентификатором "archtool.user". Для адресации объекта указывается ссылка специального формата 
"/entities/interactions/blank?id=archtool.user". Где:

* **/entities/** - сообщает SEAF.ArchTool, что обращение идет к сущности;
* **interactions** - идентификатор сущности;
* **blank** - идентификатор презентации;
* **?id=archtool.user** - передаваемые параметры в формате URL;
...
Здесь значения "SEAF.ArchTool" и "User" подсвечиваются желтым цветом как неверные. 
...
сценарий привлечения потенциального клиента SEAF.ArchTool через статьи на [Хабр](https://habr.com/ru/users/rpiontik/).

Предполагаемый сценарий перехода потенциального клиента в репозиторий проекта с Хабр:
```yaml
interactions:
  ...
  archtool.customer.promotion.article:
    ...
    steps:
      - from: habr
        to: archtool.customer
        value: Анализ новых статей
      - from: archtool.customer
        to: habr
        value: Чтение статьи о SEAF.ArchTool
      - from: archtool.customer
        to: archtool.gitlab.repository
...
interactions:
  ...
  archtool.customer.deployment:
    title: Разворачивание SEAF.ArchTool
    triggers:
      - Первое знакомство с SEAF.ArchTool
    results:
      - Потенциальный клиент (лид)
    steps:
      - from: archtool.customer
        to: habr.articles
        value: Получение информации о SEAF.ArchTool
        sub-interactions: archtool.customer.promotion.article # Здесь ссылка на взаимодействие пользователя со статьями
      - from: archtool.customer
        to: archtool.gitlab.repository
...
![](@entity/interactions/blank?id=archtool.customer.deployment)
