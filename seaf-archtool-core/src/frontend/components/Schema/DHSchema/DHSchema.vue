...
<script>
  ...
  const logger = getLoggerWithTag('f/c/S/D/DHSchema');
                ...
                if (message.result === 'OK') {
                  if (message.graph?.warnings?.length === 0) {
                    ...
                    try {
                      ...
                      logger.warn(() => 'Can\'t cache SA result:', e);
                    }
                  }
  ...
  export default {
    ...
    methods: {
      ...
      updateNodeView() {
        ...
        for(const id in map)  {
          const node = map[id];
          node.opacity = unselected || this.selected.nodes[id] ? 1 : OPACITY;
        }
      },
      ...
      updateTracksView() {
        ...
        this.presentation.tracks = this.presentation.tracks.map((track) => {
          if (unselected) {
            track.animate = false;
            track.opacity = this.lineOpacity;
          } else {
            track.highlight = !!this.selected.links[track.id];
            track.animate = track.highlight;
            track.opacity = track.animate ? this.lineOpacity : OPACITY * this.lineOpacity;
          }
          return track;
        }).sort((track1, track2) => {
      ...
      // Перестроить viewbox
      rebuildViewBox() {
        if (!this.presentation.valueBox) return;
