...
Оригинальный шаблон:
![Шаблон](../docs/templates/test.svg)
...
```yaml
 docs:
   dochub.plugins.svg_file:
     title: Проверка svg
     type: svg
     location: SEAF.ArchTool/Руководство/Документы/SVG/Пример svg
...
Результат: 
```
![svg-документ](@document/archtool.plugins.svg_file)
```
...
![svg-документ](@document/archtool.plugins.svg_file_error?check=1)
