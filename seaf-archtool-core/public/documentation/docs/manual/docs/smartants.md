...
```yaml
  ...
  archtool.smartants.examples.simple:
    location: SEAF.ArchTool/Руководство/Документы/Представления/01 Простой пример
    autor: R.Piontik
...
![Простой пример](@document/archtool.smartants.examples.simple)
...
![Цвет связей](@document/archtool.smartants.examples.colors)
...
```yaml
  ...
  archtool.smartants.examples.domains:
    location: SEAF.ArchTool/Руководство/Документы/SmartAnts/02 Зонирование диаграммы
    autor: R.Piontik
    type: smartants           
    source:                   
      config:                 
...
![Пример иерархии на диаграмме](@document/archtool.smartants.examples.domains)
...
построения диаграммы в [отдельном файле](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/docs/manual/docs/examples/sa-complex.yaml).

```yaml
  ...
  archtool.smartants.examples.complex:
    location: SEAF.ArchTool/Руководство/Документы/SmartAnts/02 Сложная диаграмма
    autor: R.Piontik
    type: smartants           
    source: examples/sa-complex.yaml   # Данные для диаграммы расположены в файле 
  ...
```

Результат:
![Пример сложной диаграммы](@document/archtool.smartants.examples.complex)
...
![Результат на примере сложной диаграммы](@document/archtool.smartants.examples.complex.hideboundaries)
...
Результат на примере сложной диаграммы:
![Результат на примере сложной диаграммы](@document/archtool.smartants.examples.complex.hideleaves)
...
Результат на примере сложной диаграммы:
![Результат на примере сложной диаграммы](@document/archtool.smartants.examples.complex.hidetitles)
...
![Получаем такую схему](@document/archtool.smartants.examples.gravity)
...
Добавим к диаграмме сценарий ее разъяснения ([содержимое файла](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/docs/manual/docs/examples/sa-anim.yaml?line=95)):
```yaml
...
# Анимация
animation:
...
![Пример анимации](@document/archtool.smartants.examples.anim)
...
nodes:
  my-symbol:
    ...
    symbol: cat
```
Исходник файла [здесь](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/docs/manual/docs/examples/sa-symbol.yaml?line=32). Результат:

![Пример собственного символа](@document/archtool.smartants.examples.custom-symbol)
...
```yaml
  ...
  archtool.smartants.examples.simple-query:
    location: SEAF.ArchTool/Руководство/Документы/SmartAnts/06 Простой запрос к данным
    autor: R.Piontik
    type: smartants           
    source: >
      (
...
![Пример простого запроса к данным](@document/archtool.smartants.examples.simple-query)

В результате, мы получили сильно упрощенный аналог диаграммы [L1](/architect/contexts/archtool). 
А также на диаграмме есть ошибка. Они возникает из-за того, что компонент "Документы" не имеет символа. 

Следующий запрос устраняет эту проблему ([исходник](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/docs/manual/docs/examples/sa-query.jsonata)):
```json
(
...
![Пример сложного запроса к данным](@document/archtool.smartants.examples.complex-query)
...
![Пример физической связи двух объектов](@document/archtool.smartants.examples.physical-connection)
...
![Пример интеграции через несколько объектов](@document/archtool.smartants.examples.through-objects)
...
![Пример обоюдотупых стрелок](@document/archtool.smartants.examples.without-tips)
