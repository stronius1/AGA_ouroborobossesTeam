...
Представление:
![BPMN](@document/archtool.example.bpmn)


Манифест:
```yaml
...
archtool.example.bpmn:
    location: SEAF.ArchTool/Руководство/Документы/BPMN/Простой пример  # Размещение документа в меню
...
В настоящий момент эта возможность доступна в [плагине к IDE IDEA](https://gitverse.ru/seafteam/seaf-archtool-ideplugin-jetbrains/releases).
...
SEAF.ArchTool может обращаться к описанию процессов в XML формате аналогично YAML и JSON. Для этого необходимо указать XML
файл как источник данных. Например, рассмотрим анализ процесса:

![BPMN](@document/archtool.example.bpmn.full)
...
![BPMN](@document/archtool.example.bpmn.analyse)