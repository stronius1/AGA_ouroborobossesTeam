...
В фукнцию может быть передана схема с использованием $rels (см. [entities](/entities/docs/blank?dh-doc-id=archtool.flex_metamodel.entities))
```json
    /* Создаём валидатор */
    $validator := $jsonschema({
      "type": "object",
...
**В настоящий момент поддерживается только в [шаблонах документов](/docs/archtool.templates).**
