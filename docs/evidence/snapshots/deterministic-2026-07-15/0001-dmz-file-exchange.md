...
agent_finding:
  severity: major
  evidence: 'pattern: file — файловый обмен вне утверждённых паттернов'
architect_action: override
architect: И. Петров, домен «Интеграции»
rationale: 'Файловый обмен в DMZ-сегменте разрешён регламентом ИБ-2024-17 для batch-выгрузок:
  канал контролируется файловым шлюзом, наблюдаемость обеспечена. Запрет PRIN-002
  относится к неуправляемому обмену внутри контура, а не к этому классу потоков.
...
proposed_mutation:
  ...
  exception:
    when:
      ...
      - field: approvals
        contains: security
    rationale: 'Файловый обмен допустим только как контролируемая batch-выгрузка через
