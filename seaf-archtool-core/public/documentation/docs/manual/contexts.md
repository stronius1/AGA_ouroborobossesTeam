...
Имеет смысл отображать [компоненты](/docs/archtool.components) архитектуры в контексте их рассмотрения. Например, если рассматривается
...
contexts:                                   # Контексты представления архитектурных компонентов
  archtool:                                 # Идентификатор контекста
        title: Общая архитектура SEAF.ArchTool     # Название контекста. Отображается в заголовке диаграммы
        location: SEAF.ArchTool/Общая архитектура  # Размещение документа в дереве навигации
...
```
Чтобы понять это лучше, рассмотрим SEAF.ArchTool в C4 Model:

![](@anchor/c4model_1)

## 1. Уровень I: Система в масштабе ее взаимодействия с пользователями и другими системами

![Система в масштабе ее взаимодействия с пользователями и другими системами](@context/archtool)

Описание контекста в манифесте [манифесте](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/arch/contexts.yaml):
```yaml
contexts:
  archtool:                                 # Идентификатор контекста
        title: Общая архитектура SEAF.ArchTool     # Название контекста
        location: SEAF.ArchTool                    # Размещение документа в дереве навигации
        components:                         # Компоненты входящие в контекст
            - archtool.front
            - archtool.user
            - archtool.plantuml
...
в самих компонентах (см. [компоненты](/docs/archtool.components)).
...
![Взаимосвязанные контейнеры](@context/archtool.front)

Описание контекста в манифесте [манифесте](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/arch/contexts.yaml?line=56):
```yaml
contexts:   # Контексты представления архитектурных компонентов
    ...
  archtool.front:
        title: Контейнерная архитектура SEAF.ArchTool
        location: SEAF.ArchTool/Контейнерная архитектура
        components:
            - archtool.browser.localstorage
            - archtool.front.spa
            - archtool.gitlab.api
            - archtool.gitlab.oauth
            - archtool.gitlab.repository
            - archtool.gitlab.gitclient
            - archtool.plantuml.jar
            - archtool.web
    ...
```
![](@anchor/c4model_3)
...
![Взаимосвязанные компоненты](@context/archtool.front.spa)

Описание контекста в манифесте [манифесте](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/arch/contexts.yaml?line=65):
```yaml
contexts:   # Контексты представления архитектурных компонентов
    ...
  archtool.front.spa:
        title: Компонентная архитектура SEAF.ArchTool
        location: SEAF.ArchTool/Компонентная архитектура
        components:
            - archtool.front.spa.*
    ...
```
...
![дополнительные сведения о дизайне архитектурных элементов](@context/archtool.sequence)

Описание контекста в манифесте [манифест](https://gitverse.ru/seafteam/seaf-archtool-core/content/master/public/documentation/arch/contexts.yaml?line=93):
```yaml
contexts:   # Контексты представления архитектурных компонентов
    ...
  archtool.sequence:
        title: Диаграмма взаимодействия SEAF.ArchTool
        location: SEAF.ArchTool/Диаграмма взаимодействия
...
переопределить источник. Сделать это можно используя поля "source" и "original", аналогично [таблицам](/docs/archtool.tables).

```yaml
    ...
    archtool.custom:
        title: Компоненты эксперта R.Piontik
        location: SEAF.ArchTool/Контекст по источнику
        ...
        components:
            - archtool.*      # Выводим все компоненты домена archtool
    ...
```
...
![Источник данных контекста](@document/archtool.context.source)


Фильтр по компонентам будет применятся уже к подготовленным данным источником. 
Результат:

![Компоненты в которых эксперт R.Piontik](@context/archtool.custom)
...
![uml опции контекста](@context/archtool.uml_options)
