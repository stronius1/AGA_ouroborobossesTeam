...
Так же плагин поддерживает параметризацию показываемой диаграммы на основе данных из манифеста archtool.
...
Это означает, что файл из репозитория перед отображением используется как шаблон для применения у нему тех данных, которые возвращает запрос из параметра source документа archtool.
...
@startuml
    ...
    :Получаем шаблон **template**; <<input>>
    :Получаем данные **data**; <<input>>
    :Получает список обработчиков handlers; <<procedure>>
    :Получаем DOM представление шаблона oDOM; <<procedure>>
    
    while (**el** - элемент из массива **data**) is (выражение)
        :Берем xpath из **el.xpath**; <<procedure>>
        :Применяем xpath к документу oDOM -> **xpath_result**; <<procedure>>
    
        while (**node** - элемент из результата **xpath_result**) is (элемент)
    
            while (**handler** - элемент обработчиков **handlers**) is (обработчик)
                :Получаем атрибуты для вызова по коду обработчика \n **attrs** = **el**.operation[handler_name]; <<procedure>>
                :Получаем параметры вызова \n **params** = **el**.params; <<procedure>>
                :вызов обработчика \n handler(**plugin**, **node**, **attrs**, **params**); <<procedure>>
                if (произошла ошибка) then (ошибка)
                    :логируем ошибку; <<output>>
                endif
            endwhile (конец)
...
1. Установить значение переменной VUE_APP_DOCHUB_PLUGIN_DRAWIO_LOAD_CUSTOM_HANDLERS перед сборкой archtool в значение true
...
```yaml
  archtool.plugins.drawio.as_is:
    location: SEAF.ArchTool/Руководство/Документы/Drawio/Файл как есть
    type: drawio
    template: templates/example.drawio
```

![](@document/archtool.plugins.drawio.as_is)

## Применение к шаблону данных из запроса

```yaml
  archtool.plugins.drawio.parametrized:
    location: SEAF.ArchTool/Руководство/Документы/Drawio/Применили пост. обработку
    type: drawio
    template: templates/example.drawio
    source: >
      (
...
![](@document/archtool.plugins.drawio.parametrized)
