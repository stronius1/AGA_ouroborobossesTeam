    ...
    onFoundLoadingError() {
        if (listeners.onFoundLoadingError) {
            request(listeners.onFoundLoadingError)
                .catch((e) => {
                    logger.error(() => `Error of delivery event onFoundLoadingError to [${listeners.onFoundLoadingError}] with error [${e.message}]`);
                });
            logger.info(() => `Sent event onFoundLoadingError to [${listeners.onFoundLoadingError}]`);
        }
    }
};
